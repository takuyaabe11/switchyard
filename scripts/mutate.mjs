...
const SUITES = {
  ...
  report: {
    ...
    mutations: [
      ...
      { name: 'R31 sleep を含む until / while のループを待つ形と見ない', file: 'src/replay/reruns.mjs', from: 'if (WAIT_LOOP.test(command) || WAIT_ALONE.test(command))', to: 'if (WAIT_ALONE.test(command))' },
  ...
  single: {
    ...
    tests: ['test/hooks/timeouts.test.mjs', 'test/hooks/failure.test.mjs', 'test/hooks/ports.test.mjs', 'test/report/report.test.mjs'],
    mutations: [
