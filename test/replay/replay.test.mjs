...
describe('replay(記録の根を読んで集計する)', () => {
  it('メインとサブエージェントの記録を読み、同じ tool_use を 1 回だけ数える', async () => {
    ...
    assert.deepEqual(r.hook, { deny: 1, ask: 0, wrap: 0, background: 2, alreadyBackground: 1, none: 2 });
  ...
  it('cwd の前方一致と、期間の始まりで絞る', async () => {
    ...
    assert.deepEqual(irc.hook, { deny: 1, ask: 0, wrap: 0, background: 1, alreadyBackground: 1, none: 2 });
...
describe('formatReport(端末に出す文面)', () => {
  ...
  it('中身が表に当たらない switchyard run は「承認を求める」に数え、既に背景でも同じ', async () => {
    const root = mkdtempSync(join(tmpdir(), 'crep-'));
    mkdirSync(join(root, 'p'));
    writeFileSync(join(root, 'p', 's.jsonl'), [bashLine({ id: 'a1', command: 'switchyard run -- echo hi' }), bashLine({ id: 'a2', command: 'switchyard run -- echo hi', bg: true })].join('\n') + '\n');
    const r = await replay({ dir: root, cwdPrefix: null, since: null, profilesFor: defaults, examples: 5, git: true });
    assert.equal(r.hook.ask, 2);
    assert.equal(r.hook.alreadyBackground, 0);
    assert.match(formatReport(r, { cwdPrefix: null, sinceDays: null, examples: 5 }), /switchyard run の中身に承認を求める: 2 件\(100\.0%\)/);
  });
