...
== H2 背景への書き換えに allow を付ける(権限の確認を飛ばす) | src/hooks/pretooluse.mjs | 赤 | tests 78 / fail 23 / cancelled 0 / pass 55
   ...
   赤: "switchyard run -- npx vitest run"
   赤: "switchyard run --lock port:4173 -- node scripts/e2e.mjs"
   赤: "switchyard run -- ./node_modules/.bin/vitest run"
   赤: "node <bin/switchyard.mjs> run --lock port:4173 -- npm run bench"
   赤: "./node_modules/.bin/vitest run"
   赤: "scripts/probe-run.sh gates npm run bench"
   赤: "npm test && ./node_modules/.bin/vitest run"
   赤: "npm run bench"
   赤: "switchyard run -- node -e 'console.log(\"measure-suite\")'"
   赤: batch を前景で打ったら、run_in_background だけを true にし、決定は付けない
   赤: switchyard run で包んだ部分は拒否しない。背景への判定は包みが要求する性格で行う
...
== H5 switchyard run で包んだ中も拒否の判定にかける | src/hooks/pretooluse.mjs | 赤 | tests 78 / fail 1 / cancelled 0 / pass 77
...
== H12 switchyard run の包みの性格と -- の後ろを見ない | src/hooks/pretooluse.mjs | 赤 | tests 78 / fail 6 / cancelled 0 / pass 72
   壊した行: (行を消した)
   赤: "switchyard run -- npx vitest run"
   赤: "switchyard run --lock port:4173 -- node scripts/e2e.mjs"
   赤: "switchyard run -- ./node_modules/.bin/vitest run"
...
== H6 考える層の中でも判定する | src/hooks/pretooluse.mjs | 赤 | tests 78 / fail 1 / cancelled 0 / pass 77
   壊した行: (行を消した)
   赤: SWITCHYARD_THINKER=1 と Bash 以外では何もしない
== H7 Stop が 2 度目の停止も差し戻す | src/hooks/session.mjs | 赤 | tests 78 / fail 1 / cancelled 0 / pass 77
   壊した行: if (env.SWITCHYARD_THINKER === '1') return null;
...
走行の後に `ps` で `switchyardd.mjs` / `.test.mjs` / `scripts/mutate.mjs` を探し、残りは無かった。
...
- **H18**: `node -e` のコードの中身で分類する形(`classifiableCommand` がインラインのコードを除かない)。赤は `node -e 'console.log("vitest run")'` と、`switchyard run` の包みの性格の行。
...
- **H12〜H14・H16(I1)**: `switchyard run` の `--` の後ろ・`bash -c` の中・`env -u` の値・`( … )` の中を見ない形。どれも表の該当する行が赤になる。
- **H15(C1)**: heredoc の本文を読み飛ばさない形。本文に `npm test` で始まる行を入れてあるので、表の heredoc の commit の行が背景に回って赤になる。
- **H5**: `switchyard run` で包んだ中も拒否の判定にかける形。H1 は変数名(`found.heavy`)に合わせてある。
