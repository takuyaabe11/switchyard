...
- 重なりによる遅れを学ばない(走行ごとの CPU 時間が取れないため)。
...
- **学習**
  ...
  - **メモリのピーク**: 直近 3 回で学ぶ。
  - **重なりによる遅れ**(0.20.0): 下の「重なっても遅くならない走行は待たせない」を参照。
  - **学習の単位**: repo(git の worktree は本体と共有)と profile の組。
    - 既定の表の profile(`default:batch` など)は、道具とサブコマンドまでで分ける(0.22.0)。例: `default:batch npm test`・`default:batch npx tsc`・`default:batch python3 -m pytest`。
    - 分け方: 先頭の語(パスは外す)と、オプションより前の名前の語を 3 語まで数える。`python -m <モジュール>` はモジュールまで数える。
    - 対象を絞っている走行は、名前の後ろに ` …` を付けて分ける。絞っているとみなすのは、パス・ファイル(`tests/a.py`・`a.py`)・テストの id(`a::b`)・絞り込みのオプション(`-k`・`-t`・`-g`・`--grep`・`--filter`・`--tests` など)。例: `pytest tests/a.py` は `default:batch pytest …`。
    - `switchyard.json` の profile は分けない(書いた人が単位を決めている)。
    - 0.21.0 までの記録にある `default:batch` の学習は、新しい単位には引き継がない。新しい単位で 2〜3 回走れば、また学ぶ。
    - 根拠: この開発の記録では、同じ `default:batch` の中に `npm test`(中央値 20.5 秒)と `npx tsc`(中央値 2.9 秒)が混ざっていた。`switchyard replay` の「学ぶ単位」の欄で、自分の記録のばらつき(今の単位と、分けた単位)を比べられる。
- **実測の空きへの詰め込み**: 予約されていても使われていないコアに、待っている `batch` を 1 本ずつ入れる。
- **重なっても遅くならない走行は待たせない**(0.20.0)
  - 測るもの: 走行の間、機械全体の CPU の使い方を 1 秒ごとに測る。そこから、その走行自身が使った分(CPU 時間 / 所要)を引き、他の処理が使っていたコア数を出す。switchyard の外の処理も含む。
  - 分け方: 他の処理が機械のコア数の 1 割未満(最低 0.5 コア)なら「静か」。他の処理が 4 分の 1 以上(最低 1 コア)で、自分と合わせてコア数の 9 割以上なら「重なった」。その間は数えない。
  - 倍率: 成功した 5 秒以上の直近 20 本のうち、静かな走行と重なった走行がそれぞれ 3 本以上あれば、所要の中央値の比(重なった / 静か)を出す。
  - 使い方: 倍率が 1.15 以下の走行は、CPU の空きが足りなくても待たせない。条件は次のすべて。
    - 走っている相手もみな倍率 1.15 以下(相手を遅らせない)。
    - 割り振りの合計が容量の 2 倍以内。
...
### 4.8.1 `&` で裏に回した重い走行を背景実行にする(0.21.0)

- **書き換えるもの**: 重い走行を含むコマンドの最後に `&` が 1 つだけあれば、`&` を外して `run_in_background` にする。例: `npm test > test.log 2>&1 &` → `npm test > test.log 2>&1`(背景実行)。
- **理由**: `&` で裏に回すと Bash の呼び出しはすぐ返り、走行が終わっても Claude に知らせが届かない。そのため Claude はログを sleep しながら見に行くループで待ちがちになる。背景実行なら、終われば Claude Code が知らせる。リダイレクトは残すので、出力の行き先は変わらない。
- **書き換えないもの**
  - `&` が途中にある形(`npm test & wait`・`npm test &` の後に別のコマンド)。
  - `&` が 2 つ以上ある形。
  - ( … ) の中だけの `&`。
  - 重い走行を含まないコマンド(`python3 -m http.server 8000 &` など)。
  - 拒否・承認を求める呼び出し。
- **権限**: Claude Code は書き換えた後のコマンドで権限を確かめる。`> test.log` のようなファイルへの書き込みは、書き換えの前と同じく承認を求められることがある。
- **止め方**: `SWITCHYARD_AMP_BACKGROUND=0`。
- **根拠**: 実物の通し(Claude Code 2.1.282)。Claude が出した `npm test > live.log 2>&1 &` が、`&` を外した背景実行になり、switchyard の順番待ちを通って終わった。Claude はログから結果を読んで答えた。
- **未確認**: 利用者の記録で、この形がどれだけあるか。`switchyard replay` の「& で裏に回した重い走行を背景実行にする」の欄で数えられる。
...
| `SWITCHYARD_AMP_BACKGROUND=0` | 有効 | 最後の `&` で裏に回した重い走行を背景実行にしない |
...
| `events.jsonl` | すべての決定とジョブ(コマンド・repo のパス・セッション id・終了コード・所要・走行の間に他の処理が使っていたコア数) |
...
- **待たせた時間の元**: 待たせたことで、それ以上の時間を取り戻せたかは測れていない。入出力待ちが中心の走行は、並べても本当はぶつからないことがある。0.20.0 から、重なりによる遅れを profile ごとに学び、遅くならない profile は待たせない。ただし次の点は確かめていない。
  - 学べるだけの「重なった」走行が、普段の使い方で集まるか。switchyard が重なりを避けるので、重なった走行は switchyard の外の負荷や詰め込みでしか生まれない。
  - 倍率の境(1.15)と「静か」「重なった」の境が妥当か。
  - 「避けた遅れの見込み」は学んだ倍率からの見積もりで、実測ではない。所要は走行ごとに変わる(テストが増える・キャッシュが効く)ので、倍率にはその揺れも混ざる。
...
- **0.18〜0.22 の機能**: 実際の利用でどれだけ時間を減らしたかは未確認。入れた後の `switchyard report` の数で確かめられる(時間切れ・延ばした・背景へ回した・待つループ・ポート)。
